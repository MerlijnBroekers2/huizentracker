...
def send_new_listings_email(new_houses: list[dict], source: str):
    ...
    subject = f"🏠 {len(new_houses)} nieuwe woning(en) gevonden op {source}!"
    ...
    for h in new_houses:
        ...
        <tr>
            ...
            <td style="padding:10px; border-bottom:1px solid #eee;">€ {h['price']}/mnd</td>
            <td style="padding:10px; border-bottom:1px solid #eee;">{h.get('surface_m2') or '?'} m²</td>
            <td style="padding:10px; border-bottom:1px solid #eee;">{h.get('bedrooms') or '?'} slpk</td>
            <td style="padding:10px; border-bottom:1px solid #eee;">
                <a href="{h['url']}" style="background:#111; color:#fff; padding:6px 12px; border-radius:6px; text-decoration:none; font-size:13px;">Bekijk</a>
            </td>
        </tr>
        """
    ...
    <html><body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif; color:#111; max-width:700px; margin:auto; padding:24px;">
        <h2 style="margin-bottom:4px;">🏠 Nieuwe woningen op {source}</h2>
        <p style="color:#555; margin-top:0;">{len(new_houses)} nieuwe woning(en) gevonden die aan je criteria voldoen.</p>
        <table style="width:100%; border-collapse:collapse; margin-top:16px;">
            <thead>
                <tr style="background:#f3f4f6; text-align:left; font-size:13px; color:#555;">
                    ...
                    <th style="padding:10px;">Oppervlak</th>
                    <th style="padding:10px;">Slaapkamers</th>
                    <th style="padding:10px;"></th>
                </tr>
            </thead>
            <tbody>{rows}</tbody>
        </table>
        <p style="margin-top:24px; font-size:12px; color:#aaa;">Huizen Tracker · automatisch bericht</p>
    </body></html>
    """

    resend.Emails.send({
        "from": "Huizen Tracker <onboarding@resend.dev>",
        "to": email_to,
        "subject": subject,
        "html": html,
    })
